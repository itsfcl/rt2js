...
exports.Shader = exports.Xorshift32 = exports.v = void 0;
class v {
    static add(a, b) {
...
Xorshift32.state = 676767;
class Shader {
    constructor(scene, ctx, resolution, fov, position, maxBounce = 2) {
        this.scene = scene;
        this.ctx = ctx;
    ...
    getRays() {
        let oa = new Array(this.resolution[1]);
        for (let i = 0; i < this.resolution[1]; i++)
            oa[i] = new Array(this.resolution[0]);
    ...
    renderColored() {
        ...
        for (let i = 0; i < imgd.data.length; i += 4) {
            ...
            let tone = (v) => Math.max(0, Math.min(1, (v * (a * v + b_)) / (v * (c * v + d) + e)));
